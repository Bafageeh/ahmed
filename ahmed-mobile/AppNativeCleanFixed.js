...
async function apiJson(path, options) {
  const res = await fetch(`${API_URL}${path}`, { headers: { Accept: 'application/json', ...(options?.headers || {}) }, ...options });
  const text = await res.text();
  let json = null;
  try { json = text ? JSON.parse(text) : {}; } catch (e) {
    const sample = text.slice(0, 80).replace(/\s+/g, ' ');
    throw new Error(`رد غير JSON من ${path}: ${sample}`);
  }
  if (!res.ok) throw new Error(json?.message || `خطأ ${res.status} من ${path}`);
  return json;
}
...
export default function AppNativeCleanFixed() {
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.tabs}>{[
        ['home', '⌂', 'الرئيسية'], ['investments', '▦', 'استثماراتي'], ['wallet', '◈', 'محفظتي'], ['more', '☰', 'مزيد'],
      ].map(([k, i, l]) => <TouchableOpacity key={k} onPress={() => setTab(k)} style={[s.tab, tab === k && s.tabOn]}><Text style={s.tabIcon}>{i}</Text><Text style={s.tabText}>{l}</Text></TouchableOpacity>)}</View>
    </View>
  );
}
...
function Ta3meed() {
  ...
  const load = async (silent = false) => {
    ...
    try {
      ...
      try { await apiJson('/ta3meed/summary'); } catch (e) { setMessage(`تم تحميل الفرص، لكن summary فيه مشكلة: ${e.message}`); }
  ...
  const filtered = useMemo(() => items.filter((it) => {
    ...
    const text = [it.reference_number, it.status, it.maturity_date, meta.category, ...(it.allocations || []).map(a => a.investor_name)].filter(Boolean).join(' ').toLowerCase();
  ...
  const apply = async () => {
    ...
    setSaving(true);
    try {
      await apiJson('/ta3meed/receipts/apply-message', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: receiptText }) });
      setReceiptText(''); setPreview(null); setModal(false); setMessage('تم اعتماد الدفعة وتوزيعها'); await load(true);
    } catch (e) { setMessage(e.message); }
  ...
  return <SafeAreaView style={s.safe}>
    ...
    <ScrollView contentContainerStyle={s.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(true); }} />}>
      ...
      {filtered.map((it) => <Card key={String(it.id)} item={it} open={expanded===it.id} onPress={() => setExpanded(expanded===it.id ? null : it.id)} />)}
...
function Card({ item, open, onPress }) { const m = metaOf(item), st = statusOf(item), receipts = item.receipts || [], allocs = item.allocations || []; return <View style={[s.card,{borderColor:st.color}]}><View style={s.cardTop}><View style={[s.pill,{backgroundColor:st.bg}]}><Text style={[s.pillText,{color:st.color}]}>{st.label}</Text></View><View style={{flex:1,alignItems:'flex-end'}}><Text style={s.code}>{item.reference_number}</Text><Text style={s.meta}>يستحق {item.maturity_date || '-'}</Text></View></View><View style={s.amounts}><Mini l="المبلغ" v={money(item.principal_amount)} /><Mini l="الربح" v={money(item.expected_profit_amount,2)} /><Mini l="المستلم" v={money(m.ta3meed_received_total,2)} /></View><TouchableOpacity style={s.details} onPress={onPress}><Text style={s.detailsText}>{open?'إخفاء':'تفاصيل وسجل الدفعات'}</Text></TouchableOpacity>{open ? <View style={s.detailsBox}><Text style={s.detail}>المتبقي: {money(m.ta3meed_remaining_amount,2)}</Text><Text style={s.sub}>سجل الدفعات</Text>{receipts.length?receipts.map(r=><Text key={r.id} style={s.detail}>{r.receipt_date} · {r.receipt_type==='full'?'سداد كلي':'سداد جزئي'} · {money(r.amount,2)}</Text>):<Text style={s.muted}>لا توجد دفعات</Text>}<Text style={s.sub}>المستثمرين</Text>{allocs.map(a=><Text key={a.id} style={s.detail}>{a.investor_name}: مستثمر {money(a.invested_amount,2)} · مستلم {money(a.received_amount,2)}</Text>)}</View>:null}</View>; }
