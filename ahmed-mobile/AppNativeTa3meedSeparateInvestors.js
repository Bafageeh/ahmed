...
export default function AppNativeTa3meedSeparateInvestors() {
  ...
  return <Ta3meedScreen items={items} setItems={setItems} message={message} setMessage={setMessage} loading={loading} refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(true); }} reload={() => load(true)} onOpenInvestors={() => setScreen('investors')} />;
}
...
function Ta3meedScreen({ items, message, setMessage, loading, refreshing, onRefresh, reload, onOpenInvestors }) {
  ...
  return <SafeAreaView style={s.safe}>
    ...
    <ScrollView contentContainerStyle={s.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
      ...
      <View style={s.hero}><Text style={s.heroTop}>شاشة تعميد</Text><Text style={s.heroTitle}>محفظة تعميد</Text><Text style={s.heroText}>تم نقل المستثمرين إلى شاشة مستقلة. افتحها من رمز 👥 أعلى الشاشة.</Text></View>
...
function InvestorsScreen({ items, loading, message, onBack, onRefresh, refreshing }) {
  ...
  const openAccount = (investor) => {
    ...
    const accountData = rows.reduce((acc, row) => {
      ...
      return acc;
    }, { ...investor, invested: 0, expectedProfit: 0, received: 0, actualProfit: 0, rows, opportunities: rows.length });
    accountData.expectedTotal = accountData.invested + accountData.expectedProfit;
    accountData.remaining = Math.max(0, accountData.expectedTotal - accountData.received);
  ...
  return <SafeAreaView style={s.safe}>
    ...
    <ScrollView contentContainerStyle={s.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
      <View style={s.hero}><Text style={s.heroTop}>حسابات تعميد</Text><Text style={s.heroTitle}>أرصدة المستثمرين</Text><Text style={s.heroText}>اضغط على أي مستثمر لفتح حسابه التفصيلي وفرصه.</Text></View>
      ...
      {filtered.map((investor) => <TouchableOpacity key={investor.code} onPress={() => openAccount(investor)} style={s.investorCard}><Text style={s.investorName}>{investor.name}</Text><Text style={s.investorLine}>الرصيد المتوقع: {money(investor.invested + investor.expectedProfit - investor.received, 2)}</Text><Text style={s.investorLine}>مستثمر {money(investor.invested, 2)} · مستلم {money(investor.received, 2)} · فرص {investor.opportunities}</Text></TouchableOpacity>)}
      {!loading && filtered.length === 0 ? <View style={s.empty}><Text style={s.emptyTitle}>لا يوجد مستثمرون مطابقون</Text></View> : null}
    </ScrollView>
    <InvestorAccountModal account={account} onClose={() => setAccount(null)} />
  </SafeAreaView>;
}
