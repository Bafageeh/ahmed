...
export default function Ta3meedScreen({ onBack }) {
  ...
  const cycleFilter = () => {
    ...
    const next = filters[(currentIndex + 1) % filters.length];
      ...
      <View style={styles.screen}>
        <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.summaryRow}>
            <SummaryCard
              icon="♙"
              iconStyle={styles.greenCircle}
              label="الاستثمارات النشطة"
              value={`${summary?.active_count ?? activeCount}`}
              suffix="استثمار"
              tint={styles.summaryGreen}
            />
            <SummaryCard
              icon="↗"
              iconStyle={styles.goldCircle}
              label="الأرباح المتوقعة"
              value={formatMoney(totalProfit)}
              prefix="ر.س"
              tint={styles.summaryGold}
            />
            <SummaryCard
              icon="▢"
              iconStyle={styles.tealCircle}
              label="إجمالي الاستثمار"
              value={formatMoney(totalInvested)}
              prefix="ر.س"
              tint={styles.summaryTeal}
            />
          </View>
            ...
            <View style={styles.listArea}>
              {filteredItems.length === 0 ? (
                ...
                <Ta3meedCard
                  ...
                  onDelete={() => showInfo('زر الحذف ظاهر لمطابقة التصميم، ولم يتم تفعيل حذف تعميد في API بعد.')}
...
function FilterSegment({ filter, active, onPress }) {
  ...
  return (
    <TouchableOpacity style={[styles.filterSegment, isAll && styles.allFilterSegment, active && styles.filterSegmentActive]} onPress={onPress} activeOpacity={0.84}>
      {isAll ? <Text style={[styles.gridIcon, active && styles.gridIconActive]}>▦</Text> : <View style={[styles.filterDot, { backgroundColor: filter.dot }]} />}
  ...
  return (
    <View style={styles.investmentCard}>
      ...
      <View style={styles.itemCenter}>
        <Text style={styles.itemTitle} numberOfLines={1}>{investmentTitle(item, index)}</Text>
      ...
      <View style={styles.itemLeft}>
        <View style={styles.amountLine}>
          ...
          <Text style={styles.amountValue}>{formatMoney(item.principal_amount)}</Text>
        </View>
        <View style={styles.profitLine}>
          <Text style={[styles.profitText, overdue && styles.profitOverdue, received && styles.profitReceived]}>
            {received ? 'ربح متحقق' : 'ربح متوقع'} {formatMoney(item.expected_profit_amount, 2)} ر.س
          </Text>
        </View>
        <View style={styles.actionsRow}>
          <CircleAction icon="⌫" tone="delete" onPress={onDelete} />
          {!received ? <CircleAction icon={receiving ? '…' : '✓'} tone="receive" onPress={onReceive} disabled={receiving} /> : <CircleAction icon="✓" tone="receive" onPress={onToggle} />}
          <CircleAction icon="✎" tone="edit" onPress={onEdit} />
          <CircleAction icon={expanded ? '⌃' : '◉'} tone="view" onPress={onToggle} />
        </View>
      </View>
      ...
      {expanded ? (
        <View style={styles.expandedArea}>
          ...
          <Text style={styles.detailText}>نسبة الربح: {asNumber(item.expected_rate).toFixed(3)}%</Text>
          <Text style={styles.detailText}>تاريخ السحب: {meta.withdrawal_date || item.start_date || '-'}</Text>
          <Text style={styles.detailText}>المسترد: {formatMoney(meta.returned_amount, 2)} ر.س</Text>
          {allocations.length ? (
            <View style={styles.allocBox}>
              <Text style={styles.allocTitle}>توزيع المستثمرين</Text>
              {allocations.map((a) => (
                <Text key={a.id} style={styles.allocText}>{a.investor_name}: {formatMoney(a.invested_amount, 2)} ر.س / ربح {formatMoney(a.expected_profit_amount, 2)}</Text>
              ))}
            </View>
          ) : null}
        </View>
...
function CircleAction({ icon, tone, onPress, disabled }) {
  return (
    <TouchableOpacity style={[styles.circleAction, tone === 'delete' && styles.deleteAction, tone === 'receive' && styles.receiveAction, tone === 'edit' && styles.editAction, disabled && styles.disabledAction]} onPress={onPress} disabled={disabled} activeOpacity={0.82}>
      <Text style={[styles.circleActionText, tone === 'delete' && styles.deleteActionText, tone === 'receive' && styles.receiveActionText, tone === 'edit' && styles.editActionText]}>{icon}</Text>
    </TouchableOpacity>
  );
}
      ...
      {investors.map((investor) => (
        <View key={investor.name} style={styles.investorRow}>
          ...
          <View style={styles.investorInfo}>
            ...
            <Text style={styles.investorText}>مجموع استثماراته: {formatMoney(investor.invested, 2)} ر.س</Text>
            <Text style={styles.investorText}>مجموع أرباحه المتوقعة: {formatMoney(investor.profit, 2)} ر.س</Text>
          </View>
        </View>
      ))}
    </View>
...
function EditForm({ editing, setEditing, saveEdit, cancel }) {
  ...
  return (
    <View style={styles.editCard}>
      <Text style={styles.panelTitle}>تعديل استثمار تعميد</Text>
      ...
      <View style={styles.formActions}>
        <TouchableOpacity style={styles.formSaveButton} onPress={saveEdit} activeOpacity={0.85}><Text style={styles.formSaveText}>حفظ التعديل</Text></TouchableOpacity>
        <TouchableOpacity style={styles.formCancelButton} onPress={cancel} activeOpacity={0.85}><Text style={styles.formCancelText}>إلغاء</Text></TouchableOpacity>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  leftTopActions: { flexDirection: 'row', alignItems: 'center', gap: 9 },
  screenTitle: { position: 'absolute', left: 0, right: 0, top: 21, textAlign: 'center', fontSize: 28, fontWeight: '900', color: '#0f2233', letterSpacing: -0.5 },
  headerIcon: { width: 46, height: 46, borderRadius: 15, backgroundColor: '#ffffff', borderWidth: 1, borderColor: '#edf0f2', alignItems: 'center', justifyContent: 'center', shadowColor: '#0f172a', shadowOpacity: 0.04, shadowRadius: 8, shadowOffset: { width: 0, height: 3 }, elevation: 1, zIndex: 2 },
  headerIconText: { color: '#081525', fontSize: 24, fontWeight: '800', lineHeight: 28 },
  headerBackText: { fontSize: 34, lineHeight: 36, marginTop: -2 },
  ...
  summaryCard: { flex: 1, minHeight: 150, borderRadius: 22, paddingVertical: 18, paddingHorizontal: 10, alignItems: 'center', borderWidth: 1, borderColor: '#e4ebea', shadowColor: '#1f2937', shadowOpacity: 0.04, shadowRadius: 10, shadowOffset: { width: 0, height: 5 }, elevation: 1 },
  summaryGreen: { backgroundColor: '#f2fbf8' },
  summaryGold: { backgroundColor: '#fffaf1' },
  summaryTeal: { backgroundColor: '#f2fbf9' },
  summaryIcon: { width: 56, height: 56, borderRadius: 28, alignItems: 'center', justifyContent: 'center', marginBottom: 16, shadowColor: '#111827', shadowOpacity: 0.14, shadowRadius: 8, shadowOffset: { width: 0, height: 4 }, elevation: 3 },
  greenCircle: { backgroundColor: '#35b95a' },
  goldCircle: { backgroundColor: '#deb33c' },
  tealCircle: { backgroundColor: '#18a99a' },
  summaryIconText: { color: '#ffffff', fontSize: 24, fontWeight: '900' },
  summaryLabel: { color: '#0f1f2e', fontSize: 15, fontWeight: '800', textAlign: 'center', marginBottom: 7 },
  valueLine: { flexDirection: 'row', alignItems: 'baseline', justifyContent: 'center', gap: 5 },
  currencyText: { color: '#0f172a', fontSize: 14, fontWeight: '800' },
  summaryValue: { color: '#0b1726', fontSize: 26, fontWeight: '900', textAlign: 'center' },
  summarySuffix: { color: '#334155', fontSize: 14, fontWeight: '700', marginTop: 2 },
  ...
  investmentCard: { minHeight: 116, backgroundColor: '#ffffff', borderRadius: 21, marginBottom: 14, paddingVertical: 16, paddingHorizontal: 14, flexDirection: 'row-reverse', alignItems: 'center', borderWidth: 1, borderColor: '#edf0f2', shadowColor: '#111827', shadowOpacity: 0.06, shadowRadius: 14, shadowOffset: { width: 0, height: 7 }, elevation: 2 },
  itemIconWrap: { width: 62, alignItems: 'flex-end', justifyContent: 'center' },
  itemIcon: { width: 58, height: 58, borderRadius: 17, alignItems: 'center', justifyContent: 'center', shadowColor: '#111827', shadowOpacity: 0.18, shadowRadius: 8, shadowOffset: { width: 0, height: 4 }, elevation: 3 },
  itemIconText: { color: '#ffffff', fontSize: 27, fontWeight: '900' },
  itemCenter: { flex: 1, alignItems: 'flex-end', justifyContent: 'center', paddingRight: 9, paddingLeft: 8 },
  itemTitle: { color: '#06111f', fontSize: 21, fontWeight: '900', textAlign: 'right', maxWidth: '100%' },
  statusPill: { marginTop: 9, color: '#087337', backgroundColor: '#eaf7e8', borderRadius: 999, overflow: 'hidden', paddingHorizontal: 16, paddingVertical: 5, fontSize: 14, fontWeight: '900', textAlign: 'center' },
  statusOverdue: { color: '#e34a19', backgroundColor: '#fff0e9' },
  statusReceived: { color: '#1267b3', backgroundColor: '#e8f2fb' },
  dateRow: { marginTop: 17, flexDirection: 'row-reverse', alignItems: 'center', gap: 6 },
  calendarIcon: { color: '#4b5563', fontSize: 14, fontWeight: '900' },
  dateText: { color: '#374151', fontSize: 14, fontWeight: '600', textAlign: 'right' },
  inlineCheck: { width: 21, height: 21, borderRadius: 11, borderWidth: 1.5, borderColor: '#0d7772', color: '#0d7772', textAlign: 'center', fontWeight: '900', lineHeight: 18, marginRight: 4 },
  itemLeft: { width: 165, alignSelf: 'stretch', justifyContent: 'space-between', alignItems: 'flex-start', paddingVertical: 1 },
  amountLine: { flexDirection: 'row', alignItems: 'baseline', gap: 6 },
  currencySmall: { color: '#0f172a', fontSize: 14, fontWeight: '800' },
  amountValue: { color: '#0a1424', fontSize: 21, fontWeight: '900' },
  profitLine: { marginTop: 2 },
  profitText: { color: '#008b54', fontSize: 16, fontWeight: '800', textAlign: 'left' },
  profitOverdue: { color: '#f05a12' },
  profitReceived: { color: '#1673ca' },
  actionsRow: { flexDirection: 'row', gap: 11, marginTop: 15, alignItems: 'center' },
  circleAction: { width: 37, height: 37, borderRadius: 19, backgroundColor: '#ffffff', borderWidth: 1, borderColor: '#e8edf1', alignItems: 'center', justifyContent: 'center' },
  circleActionText: { color: '#334155', fontSize: 17, fontWeight: '900' },
  deleteActionText: { color: '#ef1d1d' },
  receiveActionText: { color: '#14823b' },
  editActionText: { color: '#1d70d8' },
  deleteAction: { backgroundColor: '#fffefe' },
  receiveAction: { backgroundColor: '#fbfffb' },
  editAction: { backgroundColor: '#fcfdff' },
  disabledAction: { opacity: 0.5 },
  expandedArea: { position: 'absolute', left: 12, right: 12, top: 112, backgroundColor: '#f8fafc', borderRadius: 16, borderWidth: 1, borderColor: '#e2e8f0', padding: 12, zIndex: 4 },
