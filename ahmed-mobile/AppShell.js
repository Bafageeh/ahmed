...
export default function AppShell({ currentUser, onLogout }) {
  ...
  const renderScreen = () => {
    ...
    if (activeTab === 'secureVault') return <SecureVaultScreen onBack={() => openTab('reports')} />;
...
function FutureMonthlyIncomeScreen({ goTo }) {
  ...
  const loadTokenizeIncome = async () => {
    try {
