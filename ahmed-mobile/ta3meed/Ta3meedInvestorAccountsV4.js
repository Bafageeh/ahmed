...
function endedProfitAmountOf(opportunity) {
  const investorActualProfit = n(opportunity?.actual_profit_amount);
  if (investorActualProfit > 0) return investorActualProfit;

  const investorContributionProfit = n(opportunity?.contribution_profit_amount) || n(opportunity?.expected_profit_amount);
  if (investorContributionProfit > 0) return investorContributionProfit;

  const invested = n(opportunity?.invested_amount);
  const principal = n(opportunity?.principal_amount);
  const sharePercent = n(opportunity?.share_percent);
  const ratio = principal > 0 ? invested / principal : (sharePercent > 0 ? sharePercent / 100 : 0);
  const totalProfit = n(opportunity?.opportunity_actual_profit_amount) || n(opportunity?.opportunity_expected_profit);
  if (ratio > 0 && totalProfit > 0) return totalProfit * ratio;

  return Math.max(0, n(opportunity?.received_amount) - invested);
}
...
export function Ta3meedInvestorAccounts({ investors, backRequestVersion = 0, onExit }) {
  ...
  useEffect(() => {
    ...
    if (selected && selectedScreen !== 'home') return setSelectedScreen('home');
    if (selected) {
      setSelected(null);
      setSelectedScreen('home');
      return;
  ...
  if (selected) {
    return <InvestorAccount investor={selected} screen={selectedScreen} setScreen={setSelectedScreen} onBack={() => setSelected(null)} />;
  }
...
function InvestorAccount({ investor, screen, setScreen, onBack }) {
  ...
  const investorTa3meed = ta3meedInvestorAmount(account);
  const isEditing = Boolean(editingEntryId);

  const emptyAccount = () => ({ investor, balance: 0, entries: [], timeline: [], opportunities: [], activeInvested: 0, activePartialReceived: 0, endedProfit: 0, summary: {} });
  ...
  const saveEntry = async () => {
    ...
    if (!value) return setMessage('أدخل المبلغ أولًا');
    setMessage(isEditing ? 'جاري حفظ التعديل...' : (entryType === 'withdrawal' ? 'جاري تسجيل السحب...' : 'جاري إضافة الرصيد...'));
    try {
      const url = isEditing ? `${API_URL}/ta3meed/investors/${investor.code}/account/entries/${editingEntryId}` : `${API_URL}/ta3meed/investors/${investor.code}/account/entries`;
  ...
  const importBulkEntries = async () => {
    ...
    if (!parsed.length) return setMessage('لم يتم التعرف على أي سطر. الصيغة: 05-10-2024 10,000 ملاحظة');
    setBulkBusy(true);
  ...
  const deleteEntry = async (entry) => {
    ...
    try {
      const response = await fetch(`${API_URL}/ta3meed/investors/${investor.code}/account/entries/${entry.id}`, { method: 'DELETE', headers: { Accept: 'application/json' } });
      const json = await response.json();
  ...
  return (
    <View style={styles.investorScreen}>
      <TouchableOpacity style={styles.investorAccountBackButton} onPress={screen === 'home' ? onBack : goHome} activeOpacity={0.84}>
        <Text style={styles.investorAccountBackText}>{screen === 'home' ? 'رجوع لحسابات المستثمرين' : `رجوع لشاشة ${investor.name}`}</Text>
      </TouchableOpacity>

      {screen === 'home' ? <InvestorHome investor={investor} account={account} balance={balance} investorTa3meed={investorTa3meed} message={message} openManage={() => setScreen('manage')} openMovements={() => setScreen('movements')} openOpportunities={() => setScreen('opportunities')} /> : null}
      {screen === 'manage' ? <ManageBalanceScreen investor={investor} message={message} entries={entries} amount={amount} setAmount={setAmount} entryDate={entryDate} setEntryDate={setEntryDate} notes={notes} setNotes={setNotes} entryType={entryType} setEntryType={setEntryType} bulkText={bulkText} setBulkText={setBulkText} bulkBusy={bulkBusy} isEditing={isEditing} saveEntry={saveEntry} resetForm={resetForm} importBulkEntries={importBulkEntries} startEdit={startEdit} deleteEntry={deleteEntry} /> : null}
...
function InvestorHome({ investor, account, balance, investorTa3meed, message, openManage, openMovements, openOpportunities }) {
  const capital = n(balance) + n(account?.endedProfit);
  const cash = capital - n(account?.activeInvested);
  ...
  return (
    <>
      ...
      <View style={[styles.investorEntryTypeRow, { marginTop: 8 }]}>
        <MiniStat title="رأس المال" value={money(capital, 2)} />
        <MiniStat title="الكاش" value={money(cash, 2)} />
      </View>
      <View style={[styles.investorEntryTypeRow, { marginTop: 8 }]}>
        <MiniStat title="ربح متوقع" value={money(account?.expectedProfit, 2)} />
        <MiniStat title="ربح تعميد المنتهي" value={money(account?.endedProfit, 2)} />
      </View>
      <View style={[styles.investorEntryTypeRow, { marginTop: 8 }]}>
...
function ManageBalanceScreen({ investor, message, entries, amount, setAmount, entryDate, setEntryDate, notes, setNotes, entryType, setEntryType, bulkText, setBulkText, bulkBusy, isEditing, saveEntry, resetForm, importBulkEntries, startEdit, deleteEntry }) {
  return (
    <>
      ...
      <View style={styles.investorPaymentCard}>
        ...
        <TextInput value={notes} onChangeText={setNotes} placeholder="ملاحظات" placeholderTextColor="#94a3b8" style={[styles.investorPaymentInput, styles.investorNotesInput]} multiline textAlignVertical="top" />
        ...
        {isEditing ? <TouchableOpacity style={styles.investorCancelEditButton} onPress={resetForm} activeOpacity={0.84}><Text style={styles.investorCancelEditText}>إلغاء التعديل</Text></TouchableOpacity> : null}
      </View>
      ...
      <View style={styles.investorPaymentCard}>
        ...
        <TextInput value={bulkText} onChangeText={setBulkText} placeholder={'05-10-2024 10,000\n11-11-2024 15,000\n18-04-2025 -1,000 ملاحظة'} placeholderTextColor="#94a3b8" style={[styles.investorPaymentInput, styles.investorBulkInput]} multiline textAlignVertical="top" />
      ...
      {entries.length === 0 ? <Text style={styles.investorScreenSubtitle}>لا توجد حركات رصيد يدوية بعد.</Text> : entries.map((entry) => (
        <View key={entry.id} style={styles.investorPaymentCard}>
          <View style={styles.balanceEntryHeader}>
            <View style={[styles.balanceEntryActions, { gap: 8 }]}>
              <TouchableOpacity style={[styles.balanceEntryActionButton, { minWidth: 70, paddingHorizontal: 10 }]} onPress={() => startEdit(entry)} activeOpacity={0.84}><Text style={[styles.balanceEntryActionIcon, styles.balanceEntryEditIcon, { fontSize: 13 }]}>تعديل</Text></TouchableOpacity>
              <TouchableOpacity style={[styles.balanceEntryActionButton, { minWidth: 62, paddingHorizontal: 10 }]} onPress={() => deleteEntry(entry)} activeOpacity={0.84}><Text style={[styles.balanceEntryActionIcon, styles.balanceEntryDeleteIcon, { fontSize: 13 }]}>حذف</Text></TouchableOpacity>
            </View>
...
function FinancialMovementsScreen({ investor, timeline }) {
  return (
    <>
      ...
      {timeline.length === 0 ? <Text style={styles.investorScreenSubtitle}>لا توجد حركات مالية لهذا المستثمر بعد.</Text> : timeline.map((movement, index) => (
  ...
  return (
    <TouchableOpacity style={styles.investorAccountButton} onPress={onPress} activeOpacity={0.84}>
      <View style={{ flex: 1, alignItems: 'flex-end' }}>
        ...
        <Text style={styles.investorScreenSubtitle}>{text}</Text>
      </View>
      <Text style={styles.investorAccountButtonIcon}>›</Text>
    </TouchableOpacity>
  );
...
function EntryTypeButton({ label, active, onPress, danger }) {
  return (
    <TouchableOpacity
      style={[
        styles.investorEntryTypeButton,
        active && styles.investorEntryTypeButtonActive,
        active && danger && styles.investorEntryTypeButtonDanger,
      ]}
      onPress={onPress}
      activeOpacity={0.84}
    >
...
function MiniStat({ title, value }) {
  return (
    <View style={styles.investorMiniStat}>
      <Text style={styles.investorMiniStatTitle}>{title}</Text>
      <Text style={styles.investorMiniStatValue}>{value}</Text>
    </View>
  );
}
