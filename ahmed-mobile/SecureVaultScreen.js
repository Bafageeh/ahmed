...
function BankAccountsDropdown({ accounts, open, onToggle, revealedId, onReveal }) {
...
function BankCard({ item, creditDebts, revealed, onReveal, onEdit, onDelete }) {
  ...
  const credit = item.card_type === 'credit'; const brand = item.card_type === 'mada' ? 'مدى' : item.card_brand === 'mastercard' ? 'ماستركارد' : 'فيزا';
  return <View style={styles.cardBox}><View style={styles.cardHeader}><View style={styles.siteActions}><TouchableOpacity style={styles.iconAction} onPress={onEdit}><Text>✏️</Text></TouchableOpacity><TouchableOpacity style={styles.iconAction} onPress={onDelete}><Text>🗑️</Text></TouchableOpacity></View><View style={styles.cardTitleBlock}><Text style={styles.cardName}>{item.title || 'بطاقة'}</Text><Text style={styles.cardBrand}>{item.card_type === 'mada' ? 'مدى' : `ائتمانية • ${brand}`}</Text></View><NetworkBadge brand={item.card_type === 'mada' ? 'mada' : item.card_brand} /></View><View style={styles.specGrid}><Spec label="نوع البطاقة" value={item.card_type === 'mada' ? 'مدى' : `ائتمانية - ${brand}`} /><Spec label="تاريخ الكشف" value={item.statement_day ? `يوم ${item.statement_day}` : '—'} /><Spec label="الحد الائتماني" value={credit ? (debt ? money(debt.credit_limit) : 'غير مربوط') : '—'} /><Spec label="رقم سداد" value={revealed ? (item.sadad_number || '—') : (item.has_sadad_number ? '••••••' : '—')} /></View>{(item.has_sadad_number || item.has_card_number || item.card_last_four) ? <TouchableOpacity style={styles.revealButton} onPress={onReveal}><Text style={styles.revealText}>{revealed ? 'إخفاء البيانات' : 'فك بيانات البطاقة'}</Text></TouchableOpacity> : null}{revealed && item.card_number ? <SecretRow label="رقم البطاقة" value={item.card_number} /> : item.card_last_four ? <Text style={styles.lastFour}>آخر 4 أرقام: {item.card_last_four}</Text> : null}</View>;
}
...
function preparePayload(form, mode, creditDebts) {
  ...
  if (mode === 'card') {
    ...
    if (payload.card_type === 'credit') { if (!['visa', 'mastercard'].includes(payload.card_brand)) return { error: 'حدد Visa أو Mastercard.' }; const debt = creditDebts.find((entry) => String(entry.id) === String(payload.credit_card_debt_id)); if (!debt) return { error: 'اربط البطاقة بسجلها في مديونية بطائق الائتمان.' }; } else { payload.card_brand = 'mada'; payload.credit_card_debt_id = null; }
  }
  return { payload };
}
