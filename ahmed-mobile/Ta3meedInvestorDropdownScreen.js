...
export default function Ta3meedInvestorDropdownScreen({ onBack }) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        <TouchableOpacity style={styles.categoryDropdownButton} onPress={() => setCategoryOpen(true)} activeOpacity={0.85}>
      ...
      <CategoryModal
        ...
        onSelect={(value) => { setCategoryFilter(value); setCategoryOpen(false); }}
      />
      <InvestorsModal
...
function CategoryModal({ visible, selected, onClose, onSelect }) {
  const options = [['all', 'كل التصنيفات'], ...CATEGORIES.map((category) => [category, category])];
  return (
      ...
      <View style={styles.modalBackdrop}>
        <View style={styles.categoryModalCard}>
          <View style={styles.modalHeader}>
            ...
            <Text style={styles.modalTitle}>التصنيف</Text>
          </View>
          <View style={styles.categoryOptionsGrid}>
            {options.map(([key, label]) => (
              <TouchableOpacity key={key} style={[styles.categoryOption, selected === key && styles.categoryOptionActive]} onPress={() => onSelect(key)} activeOpacity={0.85}>
                <Text style={[styles.categoryOptionText, selected === key && styles.categoryOptionTextActive]}>{label}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  categoryDropdownButton: { marginTop: 14, marginBottom: 4, backgroundColor: '#ffffff', borderWidth: 1, borderColor: '#dbe3ef', borderRadius: 18, paddingHorizontal: 14, paddingVertical: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  categoryDropdownValue: { flex: 1, color: '#0f172a', fontWeight: '900', textAlign: 'right', fontSize: 15 },
  categoryDropdownArrow: { color: '#0f766e', fontWeight: '900', fontSize: 16, marginRight: 10 },
  categoryModalCard: { backgroundColor: '#fff', borderRadius: 26, padding: 16, borderWidth: 1, borderColor: '#dbe3ef' },
  categoryOptionsGrid: { flexDirection: 'row-reverse', flexWrap: 'wrap', gap: 9, marginTop: 6 },
  categoryOption: { minWidth: 74, flexGrow: 1, borderRadius: 16, borderWidth: 1, borderColor: '#dbe3ef', backgroundColor: '#f8fafc', paddingVertical: 12, alignItems: 'center' },
  categoryOptionActive: { backgroundColor: '#0f766e', borderColor: '#0f766e' },
  categoryOptionText: { color: '#334155', fontWeight: '900' },
