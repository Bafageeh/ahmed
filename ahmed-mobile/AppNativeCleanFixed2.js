...
function Card({ item, open, onPress, onDeleteReceipt, deletingId }) {
  const m = metaOf(item), st = statusOf(item), receipts = item.receipts || [], allocs = item.allocations || [];
  const expectedTotal = n(item.principal_amount) + n(item.expected_profit_amount);
  const receivedTotal = n(m.ta3meed_received_total);
  ...
  const settlementNote = m.ta3meed_settlement_note;
  return <View style={[s.card,{borderColor:st.color}]}>
    <View style={s.cardTop}><View style={[s.pill,{backgroundColor:st.bg}]}><Text style={[s.pillText,{color:st.color}]}>{st.label}</Text></View><View style={{flex:1,alignItems:'flex-end'}}><Text style={s.code}>{item.reference_number}</Text><Text style={s.meta}>يستحق {item.maturity_date || '-'}</Text></View></View>
    ...
    <View style={s.progressBox}><View style={s.progressHeader}><Text style={s.progressText}>{pct(progress)}</Text><Text style={s.progressTitle}>نسبة الاستلام</Text></View><View style={s.progressTrack}><View style={[s.progressFill,{width:`${progress}%`}]} /></View><Text style={s.progressMeta}>المتبقي {money(m.ta3meed_remaining_amount,2)} · الدفعات {receipts.length} · الجزئية {partialCount}{fullCount ? ` · كلي ${fullCount}` : ''}</Text>{lastReceipt ? <Text style={s.progressMeta}>آخر دفعة: {lastReceipt.receipt_date || '-'} · {money(lastReceipt.amount,2)}</Text> : null}{settlementNote ? <Text style={s.settlementNote}>{settlementNote}</Text> : null}</View>
    ...
    {open ? <View style={s.detailsBox}><Text style={s.detail}>المتبقي: {money(m.ta3meed_remaining_amount,2)}</Text><Text style={s.sub}>سجل الدفعات</Text>{receipts.length?receipts.map(r=><View key={r.id} style={[s.receiptLine, r.receipt_type==='full' && s.fullReceiptLine]}><View style={{flex:1,alignItems:'flex-end'}}><Text style={s.detail}>{r.receipt_date} · {r.receipt_type==='full'?'سداد كلي':'سداد جزئي'} · {money(r.amount,2)}</Text></View><TouchableOpacity disabled={deletingId===r.id} onPress={() => onDeleteReceipt(r)} style={s.deleteReceipt}><Text style={s.deleteReceiptText}>{deletingId===r.id?'...':'حذف'}</Text></TouchableOpacity></View>):<Text style={s.muted}>لا توجد دفعات</Text>}<Text style={s.sub}>المستثمرين</Text>{allocs.map(a=>{ const share = n(item.principal_amount)>0 ? (n(a.invested_amount)/n(item.principal_amount))*100 : 0; const investorExpected = n(a.invested_amount)+n(a.expected_profit_amount); const investorRemaining = Math.max(0, investorExpected-n(a.received_amount)); const actualProfit = n(a.received_amount)-n(a.invested_amount); return <Text key={a.id} style={s.detail}>{a.investor_name}: نسبة {pct(share)} · مستثمر {money(a.invested_amount,2)} · مستلم {money(a.received_amount,2)} · ربح فعلي {money(actualProfit,2)} · متبقي {money(investorRemaining,2)}</Text> })}</View>:null}
  </View>;
}
