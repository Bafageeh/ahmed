...
export default function AppNativeTa3meedInvestors() { return <Ta3meed />; }
...
function Ta3meed() {
  ...
  const load = async (silent = false) => {
    ...
    } catch (error) { setMessage(error.message || 'تعذر تحميل بيانات تعميد'); }
  ...
  const filtered = useMemo(() => items.filter((item) => {
    ...
    if (investorFilter !== 'all' && !(item.allocations || []).some((a) => (a.investor_code || a.investor_name) === investorFilter)) return false;
  ...
  }), [items, filter, investorFilter, query]);

  const totals = useMemo(() => {
    const source = investorFilter === 'all' ? items : filtered;
  ...
  }, [items, filtered, investorFilter]);

  const openInvestorAccount = (investor) => {
    const opportunities = items
      .map((item) => {
        const allocation = (item.allocations || []).find((a) => (a.investor_code || a.investor_name) === investor.code);
        if (!allocation) return null;
        return { item, allocation, receipts: item.receipts || [] };
      })
      .filter(Boolean);
    const account = opportunities.reduce((acc, row) => {
      acc.invested += n(row.allocation.invested_amount);
      acc.expectedProfit += n(row.allocation.expected_profit_amount);
      acc.received += n(row.allocation.received_amount);
      acc.actualProfit += n(row.allocation.actual_profit_amount);
      return acc;
    }, { ...investor, invested: 0, expectedProfit: 0, received: 0, actualProfit: 0, opportunities: opportunities.length, rows: opportunities });
    account.expectedTotal = account.invested + account.expectedProfit;
    account.remaining = Math.max(0, account.expectedTotal - account.received);
  ...
  const parse = async () => {
    ...
    try {
      ...
      setPreview(json.data); setMessage('تم تحليل الرسالة');
  ...
  const submitReceipt = async (confirmed = false) => {
    ...
    setReceiptText(''); setPreview(null); setModal(false); setMessage(confirmed ? 'تمت إضافة الدفعة المكررة بعد التأكيد' : 'تم اعتماد الدفعة وتوزيعها'); await load(true);
  };
  const apply = async () => {
    if (!receiptText.trim()) return setMessage('الصق رسالة تعميد أولًا');
    setSaving(true);
  ...
  const deleteReceipt = async (receipt) => {
    const doDelete = async () => { setDeletingId(receipt.id); try { await apiJson(`/ta3meed/receipts/${receipt.id}`, { method: 'DELETE' }); setMessage('تم حذف الدفعة وإعادة الحساب'); await load(true); } catch (error) { setMessage(error.message || 'تعذر حذف الدفعة'); } finally { setDeletingId(null); } };
  ...
  return <SafeAreaView style={s.safe}>
    ...
    <ScrollView contentContainerStyle={s.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(true); }} />}>
      ...
      <View style={s.hero}><Text style={s.heroTop}>نسخة React Native الجديدة</Text><Text style={s.heroTitle}>محفظة تعميد</Text><Text style={s.heroText}>فلتر المستثمرين وحساباتهم وأرصدتهم أصبحت ضمن شاشة تعميد.</Text></View>
      ...
      <Text style={s.filterTitle}>فلتر المستثمرين</Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filters}><TouchableOpacity onPress={() => setInvestorFilter('all')} style={[s.chip, investorFilter === 'all' && s.chipOn]}><Text style={[s.chipText, investorFilter === 'all' && s.chipTextOn]}>كل المستثمرين</Text></TouchableOpacity>{investors.map((investor) => <TouchableOpacity key={investor.code} onPress={() => setInvestorFilter(investor.code)} onLongPress={() => openInvestorAccount(investor)} style={[s.chip, investorFilter === investor.code && s.chipOn]}><Text style={[s.chipText, investorFilter === investor.code && s.chipTextOn]}>{investor.name}</Text></TouchableOpacity>)}</ScrollView>
      <View style={s.investorCards}>{investors.map((investor) => <TouchableOpacity key={investor.code} onPress={() => openInvestorAccount(investor)} style={s.investorCard}><Text style={s.investorName}>{investor.name}</Text><Text style={s.investorLine}>الرصيد المتوقع: {money(investor.invested + investor.expectedProfit - investor.received, 2)}</Text><Text style={s.investorLine}>مستثمر {money(investor.invested, 2)} · مستلم {money(investor.received, 2)}</Text></TouchableOpacity>)}</View>
    ...
    <InvestorAccountModal account={investorAccount} onClose={() => setInvestorAccount(null)} />
  </SafeAreaView>;
}
...
function InvestorAccountModal({ account, onClose }) { if (!account) return null; return <Modal visible transparent animationType="slide" onRequestClose={onClose}><SafeAreaView style={s.accountWrap}><View style={s.accountHeader}><TouchableOpacity onPress={onClose} style={s.closeChip}><Text style={s.closeChipText}>إغلاق</Text></TouchableOpacity><Text style={s.accountTitle}>حساب {account.name}</Text></View><ScrollView contentContainerStyle={s.accountContent}><View style={s.accountHero}><Text style={s.accountHeroTitle}>الرصيد المتوقع</Text><Text style={s.accountHeroValue}>{money(account.remaining, 2)}</Text></View><View style={s.grid}><Metric t="إجمالي الاستثمار" v={money(account.invested,2)} /><Metric t="الربح المتوقع" v={money(account.expectedProfit,2)} /><Metric t="المستلم" v={money(account.received,2)} /><Metric t="الربح الفعلي" v={money(account.actualProfit,2)} /><Metric t="عدد الفرص" v={String(account.opportunities)} wide /></View><Text style={s.section}>فرص المستثمر</Text>{account.rows.map(({ item, allocation }) => <View key={`${item.id}-${allocation.id}`} style={s.accountRow}><Text style={s.accountRowTitle}>{item.reference_number}</Text><Text style={s.accountRowText}>الحالة: {statusOf(item).label}</Text><Text style={s.accountRowText}>مستثمر: {money(allocation.invested_amount,2)} · متوقع: {money(allocation.expected_profit_amount,2)}</Text><Text style={s.accountRowText}>مستلم: {money(allocation.received_amount,2)} · ربح فعلي: {money(allocation.actual_profit_amount,2)}</Text></View>)}</ScrollView></SafeAreaView></Modal>; }
